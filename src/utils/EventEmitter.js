export default class {
  ...
  on(_names, callback) {
    ...
    names.forEach(_name => {
      ...
      let tempCallback = that.callbacks[name.namespace];

      // Create namespace if not exist
      if(!(tempCallback instanceof Object)) {
        tempCallback = {}
      }

      // Create callback if not exist
      if(!(tempCallback[name.value] instanceof Array)) {
        tempCallback[name.value] = []
      }

      // Add callback
      tempCallback[name.value].push(callback)
    })
